# Beast of the Sea
## Tasha’s Cauldron of Everything

```statblock
layout: Basic 5e Layout
image: [[Beast of the Sea BGR PNG.png]]
name: Beast of the Sea
size: Medium
type: [[Beast]]
alignment: Unaligned
ac: 13 + PB (Natural Armor)
hp: 5 + five times your ranger level (the beast has a number of Hit Dice [d8s] equal to your ranger level)
speed: 5 ft., Swim 60 ft.
stats: [14, 14, 15, 8, 14, 11]
senses: [[Darkvision]] 60Ft, Passive Perception 12, Passive Insight 12, Passive Stealth 12
languages: Understands the languages you speak
traits:
  - name: Amphibious.
    desc: The beast can breathe both air and water.  
  - name: Primal Bond.
    desc: You can add your proficiency bonus to any ability check or saving throw that the beast makes.  
actions:
  - name: "Binding Strike."
    desc: "_Melee Weapon Attack:_ your spell attack modifier to hit, reach 5 ft., one target. _Hit:_ 5 (1d6+2) + PB piercing damage or 5 (1d6+2) + PB bludgeoning damage (your choice), and the target is [[grappled]] (escape DC equal to your spellcasting save DC). Until this grapple ends, the beast can't use this attack on another target." 
```