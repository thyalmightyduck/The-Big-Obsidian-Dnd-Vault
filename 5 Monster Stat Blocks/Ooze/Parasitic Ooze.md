# Parasitic Ooze 
Heliana’s Guide to Monster Hunting

```statblock
layout: Basic 5e Layout
name: Parasitic Ooze
size: Tiny
type: [[Ooze]]
alignment: Unaligned
ac: 8 + PB (natural armour)
hp: 5 + three times your warlock level
speed: 15 ft., Climb 15 ft.
stats: [11, 6, 16, 7, 10, 5]
saves: 
  - DEX: -2 + PB
  - CON: +3 + PB
damage_resistances: Necrotic
condition_immunities: [[Blinded]], [[Charmed]], [[Deafened]], [[Exhaustion]], [[Frightened]], [[Prone]]
skillsaves:
  - Stealth: -2 + 2 x PB
senses: Passive Perception 10, Passive Insight 10
languages: Understands the languages you speak
traits:
  - name: Amorphous
    desc: The ooze can move through a space as narrow as 1 inch wide without squeezing.
  - name: False Appearance.
    desc: While the ooze remains motionless, it is indistinguishable from an oily pool or wet rock.
  - name: Spider Climb.
    desc: The ooze can climb difficult surfaces, including upside down on ceilings, without needing to make an ability check.      
actions:
  - name: "Pseudopod"
    desc: "_Melee Weapon Attack:_ your spell attack modifier to hit, reach 5 ft., one target. _Hit:_ 1d6 + PB bludgeoning damage"  
```